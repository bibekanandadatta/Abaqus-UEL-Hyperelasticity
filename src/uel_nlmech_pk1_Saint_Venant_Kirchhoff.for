...
! **********************************************************************
     ...
     & PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
      ...
      integer               :: inod    ! Node index (changed to 6-char)
      integer               :: idim    ! Dimension index (changed to 6-char)
     ...
     &          msg='F-bar is not available: ', ivec=[jtype, nInt])
        ...
        if (matID .eq. 1) then
          call umatSaintVenant(kstep,kinc,time,dtime,nDim,analysis,
...
! **********************************************************************
      ...
      subroutine umatSaintVenant(kstep,kinc,time,dtime,nDim,analysis,
     &            nstress,nNode,jelem,coords,intpt,props,nprops,
     ...
     &            npredf,stressPK1,Amat,dPdFTensor)
      ...
      ! material properties
      real(wp)              :: Gshear, Kappa, lam_L, Emodulus, poisson
	  real(wp)              :: mu_lame, lambda_lame

      ! local kinematic variables (3x3 if tensors)
      real(wp)              :: detF, trE
      real(wp)              :: C(3,3), dPK2dFij(3,3)
      real(wp)              :: term_pk2(3,3)
      ...
      real(wp)              :: stressTensorCauchy(3,3)
	    real(wp)              :: aux1(3,3), aux2(3,3), aux3(3,3)
	    real(wp)              :: dEnjdFkl(3,3,3,3)
	    real(wp)              :: dSnjdFkl(3,3,3,3)
	    real(wp)              :: term1, term2, term3
      real(wp)              :: term5, term6, tracepart
	    real(wp)              :: dPterm, term4
      real(wp)              :: traceterm1
	    real(wp)              :: delta(3,3)  ! Kronecker delta
	  ...
	  !write(*,*) 'F = ', F
	    ...
	    mu_lame    = Emodulus/ (two*(one+poisson))
      lambda_lame = poisson*Emodulus / ((one-two*poisson)*(one+poisson))
      ...
      if (detF .le. zero) then
        call msg%ferror(flag=error, src='umatSaintVenant',
     ...
     &          ivec=[jelem, intpt], ra= detF)
      ...
      trE	=	trace(strainTensorLagrange)
	    term_pk2		= mu_lame*two*strainTensorLagrange
	    stressTensorPK2     = term_pk2 + lambda_lame*trE*ID3
      ...
      do i = 1, 3
        do j = 1, 3
		      aux1  = matmul(ID3(:,j:j), F(i:i,:))
		      aux2  = mu_lame*transpose(aux1)
		      dPK2dFij     = ID3*lambda_lame*F(i,j) + mu_lame*aux1 + aux2
		      aux3 = matmul(ID3(:,i:i), stressTensorPK2(j:j,:))
          dPdFTensor(i,j,:,:)     = aux3 + matmul(F, dPK2dFij)
     ...
     &            msg='Wrong analysis.', ch=analysis)
      ...
      end subroutine umatSaintVenant
...
! **********************************************************************
     ...
     & PREDEF(2,NPREDF,NNODE),LFLAGS(*),JPROPS(*)
      ...
      logical, parameter  :: dbgMode = .true.
     ...
     &            msg='Element is unavailable.', ia=jtype )
      ...
      if (.not. allocated(globalPostVars)) then
        ...
        call msg%finfo('-------- PK-I TOTAL LAGRANGIAN --------')
        call msg%finfo('---------------------------------------')
